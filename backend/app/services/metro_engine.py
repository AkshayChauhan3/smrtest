...
def build_trip_schedule(stations_raw: list, runtime_sec: int, direction: str):
    ...
    total_travel = max(runtime_sec - total_dwell, runtime_sec)
...
def get_train_state(train: dict, now: datetime) -> dict:
    ...
    if elapsed_s > train["trip_duration"]:
        ...
        return _waiting_at_terminal(train, now, future[0]) if future else _not_running(train, now)

    schedule = train["schedule"]
    status   = "IN_TRANSIT"
    cur_idx  = 0
    prev_station = next_station = arrived_at = departs_at = None
    eta_sec  = 0
    elapsed_in_dwell = 0
    is_in_transit    = True
