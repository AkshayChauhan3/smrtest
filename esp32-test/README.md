# ESP32 Passenger Counter Hardware & Design Documentation
...
- [Components & Hardware Requirements](#components--hardware-requirements)
- [GPIO Mapping & Pin Assignment](#gpio-mapping--pin-assignment)
...
The dual-sensor arrangement determines passenger movement direction through a metro coach door:
...
- **Sensor 1 → Sensor 2 = Passenger Entry / Boarding (occupancy +1)**
- **Sensor 2 → Sensor 1 = Passenger Exit / Alighting (occupancy -1)**

The ESP32 edge node publishes occupancy changes over serial at 115,200 baud, which are relayed by `serial_bridge.py` to the SmartRail OS backend to be injected into the live station display tables.

---

## Components & Hardware Requirements

- **ESP32-WROOM-32** (or any ESP32 Dev Module with USB-serial) — Quantity: 1
- **2 × HC-SR04 Ultrasonic Sensors** (2 cm – 400 cm range, 5 V VCC) — Quantity: 2
- **USB Power Supply / Micro-B or USB-C cable** (Data-capable, not charge-only) — Quantity: 1
- **Breadboard** (Full-size recommended) — Quantity: 1
- **Jumper wires** (M-M and M-F) — Quantity: ~12
...
### Sensor 1 (Entry Side / Platform)
- **TRIG** → GPIO4 (Output): Trigger pulse for sensor 1
- **ECHO** → GPIO14 (Input, via divider): Echo response from sensor 1

### Sensor 2 (Exit Side / Inside Coach)
- **TRIG** → GPIO27 (Output): Trigger pulse for sensor 2
- **ECHO** → GPIO33 (Input, via divider): Echo response from sensor 2
